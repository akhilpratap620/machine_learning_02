from Airfoil_prediction.entity.config_entity import DataIngestionConfig , DataTransformationConfig , DataValidationConfig,   \
...
class Configuration:
    ...
    def get_data_transformation_config(self)-> DataTransformationConfig:
        try:
            artifact_dir =self.training_pipeline_config.artifact_dir
            data_transformation_artifact=os.path.join(artifact_dir ,DATA_TRANSFORMATION_ARTIFACT_DIR, self.time_stamp)
            data_tranformation_info=self.config_info[DATA_TRANSFORMATION_CONFIG_KEY]

            transformed_train_dir=os.path.join(
                data_transformation_artifact,
                data_tranformation_info[DATA_TRANSFORMATION_DIR_NAME_KEY],
                data_tranformation_info[DATA_TRANSFORMATION_TRAIN_DIR_NAME_KEY])

              
            transformed_test_dir =os.path.join(
                data_transformation_artifact,
                data_tranformation_info[DATA_TRANSFORMATION_DIR_NAME_KEY],
                data_tranformation_info[DATA_TRANSFORMATION_TEST_DIR_NAME_KEY]) 

            transformed_preprocessed_file_path=os.path.join(
                data_transformation_artifact,
                data_tranformation_info[DATA_TRANSFORMATION_PREPROCESSING_DIR_KEY],
                data_tranformation_info[DATA_TRANSFORMATION_PREPROCESSED_FILE_NAME_KEY])
            
            data_transformed_config=DataTransformationConfig(
                transformed_train_dir=transformed_train_dir,
                transformed_test_dir=transformed_test_dir,
                preprocessed_object_file_path=transformed_preprocessed_file_path
                )
            return data_transformed_config  
            logging.info(f"data_transformation_config-[{data_transformed_config}]")  
    ...
    def get_model_trainer_config(self)-> ModelTrainerConfig:
        try:
            artifact_dir =self.training_pipeline_config.artifact_dir
            model_trainer_artifact=os.path.join(artifact_dir ,MODEL_TRAINER_ARTIFACT_DIR, self.time_stamp)
            model_trainer_info=self.config_info[MODEL_TRAINER_CONFIG_KEY]

            trained_model_dir=os.path.join(model_trainer_artifact,model_trainer_info[MODEL_TRAINER_TRAINED_MODEL_DIR_KEY])
